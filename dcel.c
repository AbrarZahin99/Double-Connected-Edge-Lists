...
// Returns a bisector struct from two points
struct bisector *getBisector(double x1, double y1, double x2, double y2){
    /* Fill in */

    struct bisector *p = NULL;

    p = malloc(sizeof(struct bisector));
...
char *getBisectorEquation(struct bisector *b){
    ...
    */
    ...
    if(b->isVertical){
        ...
        assert(returnString);
        sprintf(returnString, "x = %lf", b->bMx);
    } else {
        /* Find out memory needed. */
        int stringLength = snprintf(returnString, 0,
                                    "y = %lf * (x - %lf) + %lf", b->gradient, b->bMx,
                                    b->bMy);
        returnString = (char *) malloc(sizeof(char) * (stringLength + 1));
        assert(returnString);
...
enum intersectType intersects(struct halfEdge *he, struct bisector *b,
    ...
    double bSx, bSy, bEx, bEy;
    /* Bisector x, y pair */
    if(!b->isVertical){
        bSx = b->bMx-minLength ;
        bSy = b->bMy-minLength*b->gradient ;
        bEx = b->bMx+minLength ;
        bEy = b->bMy+minLength*b->gradient ;
    }
    if(b->isVertical){
        bSx = b->bMx ;
        bSy = b->bMy-minLength;
...
char *getIntersectionString(struct intersection *intersection){
    ...
    */
    ...
    char *returnString = NULL;
    if(intersection->sEdgeIndex <= intersection->eEdgeIndex){
        /* Find out memory needed. */
        int stringLength = snprintf(returnString, 0,
                                    "From Edge %d (%lf, %lf) to Edge %d (%lf, %lf)",
                                    intersection->sEdgeIndex, intersection->sX, intersection->sY,
                                    intersection->eEdgeIndex, intersection->eX, intersection->eY);
        returnString = (char *) malloc(sizeof(char) * (stringLength + 1));
        assert(returnString);
...
// Finds two intersection points of a bisector in the given face
struct intersection *getIntersection(struct bisector *b, struct DCEL *dcel, int face,
        double minLength){
    /*
    FILL IN */
    struct intersection *p = NULL;
    p = malloc(sizeof(struct intersection));
    struct halfEdge *start = (dcel->faces)[face].he;
    int first = 1;
    int firstIntersectionFound = 0;
    struct halfEdge *current = start;
    while(start != current || first){
        // loops until 2 intersections are found
        double sX, sY, eX, eY;
        if((firstIntersectionFound) && intersects(current, b, dcel, minLength, &(eX), &(eY))){
            p->eEdgeIndex = current->edge;
            p->eX = eX;
            p->eY = eY;
            break;
        }
        if((intersects(current, b, dcel, minLength,&(sX) , &(sY))) && !firstIntersectionFound){
            p->sEdgeIndex = current->edge;
            firstIntersectionFound = 1;
            p->sX = sX;
            p->sY = sY;
        }
...
// Calculates the diameter of a given face
double getDiameter(struct DCEL *dcel, int faceIndex){
    /*
    Fill in
    */
    if(dcel->faces[faceIndex].wt != NULL){
        double distance = 0;
        struct halfEdge *start1 = (dcel->faces)[faceIndex].he;
        struct halfEdge *start2 = (dcel->faces)[faceIndex].he;
        int first1 = 1;
        struct halfEdge *current1 = start1;
        while(start1 != current1 || first1){
            // loops through all vertices of face 1
            struct halfEdge *current2 = start2;
            int first2 = 1;
            while(start2 != current2 || first2){
                // loops through all vertices of face 2
                double newDistance = getDistance(dcel->vertices[current1->startVertex].x,
                                                 dcel->vertices[current1->startVertex].y,
                                                 dcel->vertices[current2->startVertex].x,
                                                 dcel->vertices[current2->startVertex].y);
...
// Adds watchtowers one at a time into the given dcel
void incrementalVoronoi(struct DCEL *dcel, struct watchtowerStruct *wt){
    ...
    */
    ...
    // Adding first watchtower to face 0
    ...
    // Adding second watchtower
    else if(((dcel->facesUsed) == 1) && dcel->faces[0].wt != NULL){
        struct vertex *vertex1 = (struct vertex*) malloc(sizeof(struct vertex));
        struct vertex *vertex2 = (struct vertex*) malloc(sizeof(struct vertex));
        struct intersection *intersect = (struct intersection*) malloc(sizeof (struct intersection));
        struct split *spl = (struct split*) malloc(sizeof(struct split));
        intersect = getIntersection(getBisector(wt->x, wt->y,(dcel->faces)[0].wt->x, (dcel->faces)[0].wt->y ),
                                    dcel, 0, DEFAULTMINLENGTH);
        // Creates vertex for use in getRelativeDir
        vertex1->x = intersect->sX;
        vertex1->y = intersect->sY;
        vertex2->x = intersect->eX;
        vertex2->y = intersect->eY;
        if(getRelativeDir(wt->x,wt->y, vertex1, vertex2) == OUTSIDE){
            // Applies the split, ensuring that the new wt stays outside
            spl->startSplitPoint = *vertex1;
            spl->endSplitPoint = *vertex2;
            spl->verticesSpecified = 1;
            spl->startEdge = intersect->sEdgeIndex;
            spl->endEdge = intersect->eEdgeIndex;
        }
        else{
            spl->startSplitPoint = *vertex2;
            spl->endSplitPoint = *vertex1;
            spl->verticesSpecified = 1;
            spl->startEdge = intersect->eEdgeIndex;
            spl->endEdge = intersect->sEdgeIndex;
        }
        applySplit(spl, dcel);
        (dcel->faces)[1].wt = wt;
        wt->face = 1;
    ...
    // Adding 3+ watchtowers
        ...
        for(int i =0; i<= dcel->facesUsed;i++){
            if(inFace(dcel, wt->x, wt->y, i)){
                // Checks which face the wt is in
                intersect = getIntersection(getBisector(wt->x, wt->y,(dcel->faces)[i].wt->x, (dcel->faces)[i].wt->y ),
                                            dcel, i, DEFAULTMINLENGTH);
                vertex1->x = intersect->sX;
                vertex1->y = intersect->sY;
                vertex2->x = intersect->eX;
                vertex2->y = intersect->eY;
                ...
                startEdge = spl->startEdge;
                lastFace = i;
                break;
            }
        }
        currFace = dcel->facesUsed-1;
        int storedFace = currFace;

        // For additional splits with the same watchtower when n>=3, the additional conditions in the while loop was
        // added to account for some specific edge cases which made the program to crash
        while(spl->endEdge != startEdge && (dcel->edges[spl->endEdge]).halfEdge->pair != NULL
        && (dcel->edges[spl->endEdge]).halfEdge != NULL && dcel->edges[spl->endEdge].halfEdge->face != -1 &&
        (dcel->edges[spl->endEdge]).halfEdge->pair->face != -1 && currFace != storedFace) {

            // Checks which face to jump to
            if(((dcel->edges[spl->endEdge]).halfEdge->face == currFace) ||
            ((dcel->edges[spl->endEdge]).halfEdge->face == lastFace)){
                lastFace = currFace;
                currFace = (dcel->edges[spl->endEdge]).halfEdge->pair->face;
            }
            else{
                lastFace = currFace;
                currFace = (dcel->edges[spl->endEdge]).halfEdge->face;
            }

            // Moves forward as long as we are not back to our starting face
            if(storedFace != currFace){
                intersect = getIntersection(getBisector(wt->x, wt->y,(dcel->faces)[currFace].wt->x,
                                                    (dcel->faces)[currFace].wt->y ),dcel, currFace, DEFAULTMINLENGTH);
                ...
                // The cleanup code, where the code traverses through each half edge of the newly created face
                struct halfEdge *start = (dcel->faces)[dcel->facesUsed-1].he;
                int first = 1;
                struct halfEdge *current = start;
                while((start != current || first)){
                    int boolean = 0;
                    // Pink half-edge
                    if((current->pair != NULL)&& (dcel->faces[current->pair->face].wt == NULL) &&
                    (current->next->pair == NULL || dcel->faces[current->next->pair->face].wt != NULL)){
                        (current->next)->prev = (current->pair)->prev;
                        current->prev->next = current->pair->next;
                        current->face = NOFACE;
                        boolean = 1;
                    }
                    //Orange half-edge
                    if((current->pair != NULL) && (dcel->faces[current->pair->face].wt == NULL) &&
                    (current->prev->pair == NULL || dcel->faces[current->prev->pair->face].wt != NULL)){
                        (current->pair->prev)->next = current->next;
                        current->pair->next->prev = current->prev;
                        current->pair->face = NOFACE;
                        boolean = 1;
                    }
                    // Blue half-edge
                    if((current->pair != NULL)&& dcel->faces[current->pair->face].wt != NULL){
                        current->face = dcel->facesUsed-2;
                        boolean = 1;
                    }
                    // Green/Black half-edge
                    if(!boolean){
                        current->face = dcel->facesUsed-2;
                    }
                    // Takes care of some weird half-edges created where startvertex and endvertex are same
                    if(current->startVertex == current->endVertex){
                        current->prev->next = current->next;
                        current->next->prev = current->prev;
                        current->pair->prev->next = current->pair->next;
                        current->pair->next->prev = current->pair->prev;
                    }
                    current = current->next;
                    first = 0;
